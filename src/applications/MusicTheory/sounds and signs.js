...
function getSoundDegreesAndSemitones(soundName) {
  const { degrees: soundDegrees, semitones: soundSemitones } = sounds.find(
...
function getSignSemitones(signName) {
  const { semitones: signSemitones } = signs.find(
...
function getSoundAndSign(soundDegrees, soundSemitones) {
  const { name: soundName, semitones } = sounds.find(
    (sound) => sound.degrees === soundDegrees
  );
  const { name: signName = "" } = signs.find(
    (sign) => sign.semitones === soundSemitones - semitones
  );
